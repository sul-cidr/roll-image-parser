...
class RollOptions {
	...
	protected: // (maybe make private, but will have to create accessor functions)
		...
		// m_tempo_acceleration_per_foot: the roll acceleration emulation.  This is the percentage
		// added to the tempo BPM for after the first foot of the roll.  Value of 0.22 is from
		// Wayne Stankhe.  Note the the acceleration tempo is different from the roll tempo.
		// The roll tempo is stored in the MIDI file ticks-per-quarter-note value, which represents
		// the lines of the image per second (i.e., without acceleration emulation).
		// The tempo always starts at 60 bpm (equal to the pixel rows per second in the header),
		// and the acceleration emulation is implemented at the specified rate (0.22%) at time
		// quanta equivalent to the length of time (in ticks = distance on the roll) it takes
		// for the roll to traverse the first foot (before the first acceleration adjustment
		// is applied).
		double m_tempo_acceleration_per_foot = 0.0022;
