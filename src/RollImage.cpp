...
void RollImage::assignMidiKeyNumbersToHoles(void) {
	...
	// Find the tracker bar index of the last hole on the roll
	vector<int> lastHole(trackerArray.size(), 0);
	vector<int> midiKey(trackerArray.size(), 0);
	ulongint lastHoleRow = 0;
	ulongint lastHoleMidi = 0;
	...
	for (int i=0; i<(int)trackerArray.size(); i++) {
		...
		lastHole[i] = trackerArray[i].back()->origin.first;

		if (lastHole[i] > lastHoleMidi) {
			lastHoleRow = lastHole[i];
			lastHoleMidi = trackerArray[i][0]->midikey;
		}
	}
	...
	for (int i=targetindex-2; i<=targetindex+2; i++) {
		if (lastHole[i] > lastHole[bestFitIndex]) {
			bestFitIndex = i;
		}
	}
