...
void RollImage::generateHoleMidifile(MidiFile& midifile) {
	...
	midifile.addController(m_treble_track,     tick, m_treble_ch, 10, 96); // treble notes pan rightish
        ...
        // PMB Some rolls have "first" holes that are not music related; possibly they are
        // rewind related. If the position of this hole is used as "mintime" for MIDI
        // generation, the alignment of the MIDI ticks and hole pixel positions will be
        // incorrect. Should this be corrected here, or only for the MIDI notes file
        // (generateMidifile()) or both???
        ulongint mintime = holes[0]->origin.first;
        //ulongint mintime = firstMusicRow;
        //cerr << "DEBUG generateHoleMidifile() firstMusicRow is " << firstMusicRow << " mintime is " << mintime << endl;
	ulongint maxtime = 0;
...
void RollImage::generateMidifile(MidiFile& midifile) {
	...
	midifile.addController(m_treble_track,     tick, m_treble_ch,    10, 96); // treble notes pan rightish
        ...
        // incorrect. Should this be corrected here, or only for the MIDI notes file
        // (generateMidifile()) or both???
        //cerr << "DEBUG holes[0]->isMusicHole() " << holes[0]->isMusicHole() << endl;
        //cerr << "DEBUG holes[0]->origin.first " << holes[0]->origin.first << endl;

        //ulongint mintime = holes[0]->origin.first;
        ulongint mintime = firstMusicRow;
        //cerr << "DEBUG generateMidifile() firstMusicRow is " << firstMusicRow << " mintime is " << mintime << endl;
	ulongint maxtime = 0;
	...
	for (ulongint i=0; i<holes.size(); i++) {
                // PMB
                //if (!holes[i]->isMusicHole()) {
                //        cerr << "DEBUG skipping hole " << i << " because it's not a music hole " << endl;
                //        continue;
                //}
		if (!holes[i]->attack) {
			continue;
